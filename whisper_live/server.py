...
class ClientManager:
    ...
    def is_client_timeout(self, ws: WebSocket):
        ...
        if elapsed_time >= self.max_connection_time:
            ...
            logging.warning(f"Client with uid '{self.clients[ws].client_uid}' disconnected due to overtime.")
            return True
        return False
...
#     def get_audio_from_websocket(self, ws):
#         frame_data = ws.recv()
#         if frame_data == b"END_OF_AUDIO":
#             return False
#         return np.frombuffer(frame_data, dtype=np.float32)

#     def handle_new_connection(self, ws, faster_whisper_custom_model_path):
#         try:
...
#     def process_audio_frames(self, ws):
#         frame_np = self.get_audio_from_websocket(ws)
...
#     def recv_audio(self, ws, backend: BackendType = BackendType.FASTER_WHISPER, faster_whisper_custom_model_path=None):
#         self.backend = backend
#         if not self.handle_new_connection(ws, faster_whisper_custom_model_path):
#             return

#         try:
#             while not self.client_manager.is_client_timeout(ws):
#                 if not self.process_audio_frames(ws):
#                     break
#         except Exception as e:
...
#     def run(self, host="0.0.0.0", port=9090, faster_whisper_custom_model_path=None):
#         """Start the WebSocket server using socketify."""
#         app = App()

#         @app.ws("/*")
#         def websocket(ws):
#             logging.info("New WebSocket connection")
#             ws.on_message(self.websocket_message_handler)  # Passes the message handler to WebSocket
#             ws.on_close(self.websocket_close_handler)  # Handle WebSocket closing event
...
#     def voice_activity(self, ws, frame_np):
...
#                 if not client.eos:
#                     client.set_eos(True)
#                 time.sleep(0.1)  # Sleep 100ms; wait for some voice activity.
#             return False
#         return True
